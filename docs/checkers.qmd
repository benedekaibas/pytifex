...
| **pyrefly** | 0.60.0 | `pyrefly check <file>` | [github.com/facebook/pyrefly](https://github.com/facebook/pyrefly) |
| **zuban** | 0.3.0 | `zuban check <file>` | [github.com/zubanls/zuban](https://github.com/zubanls/zuban) |
| **ty** | 0.0.1-alpha.32 | `ty check <file>` | [github.com/astral-sh/ty](https://github.com/astral-sh/ty) |

All four are configured in a single dict in [`config.py`](https://github.com/benedekaibas/pytifex/blob/main/src/pytifex/config.py):
...
Each checker has a different output format, so Pytifex uses checker-specific parsing in `_checker_reports_error()` (defined in both
`comprehensive_eval.py` and `rederive_statuses.py`). The function returns `True` if the checker reported a type error, `False` otherwise.
